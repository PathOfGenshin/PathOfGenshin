...
interface DropdownSelectorProps<T> {
  ...
  buttonValue: (t: T) => React.ReactNode
  optionValue: (t: T) => React.ReactNode
  onSelected: (t: T) => void
  disabled?: boolean
}
        ...
        {({ open }) => (
          <div className="flex items-center space-x-4">
            ...
            <div className={clsx("relative", WIDTH_SIZES[width])}>
              <Listbox.Button
                className={clsx(
                  "relative py-2 pr-8 pl-3 w-full text-sm text-left rounded-full ring-inset shadow-md transition duration-100 cursor-default bg-g-dark-1",
                  !disabled
              ...
              <Transition
                ...
                leaveTo="opacity-0"
              >
                <Listbox.Options className="overflow-auto absolute z-10 p-1 mt-1 w-full text-sm rounded-xl ring-1 ring-black ring-opacity-5 shadow-lg max-h-47 bg-g-dark-600 focus:outline-none">
                    ...
                    >
                      {({ selected }) => (
                        <>
                          <span
                            className={clsx(
                              "block pr-7 align-middle truncate",
